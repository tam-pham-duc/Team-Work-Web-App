...
const ACTION_LABELS: Record<string, string> = {
  ...
  time_log_deleted: 'removed a time entry',
  ...
  const getChangeDescription = (item: ActivityItem) => {
    if (item.action === 'updated' && item.changes.old && item.changes.new) {
      ...
      if (oldData.priority !== newData.priority) {
        changes.push(`priority from "${oldData.priority}" to "${newData.priority}"`);
      }
      if (oldData.assignee_id !== newData.assignee_id) {
        changes.push(newData.assignee_id ? 'assignee' : 'removed assignee');
      }
    ...
    if (item.action === 'timer_stopped' && item.changes.duration_minutes) {
      return `Logged ${item.changes.duration_minutes} minutes`;
    }
